...
#define NODE_ADDRESS  5,1,1,1,0x8E,0x60  // must be unique from an address space owned by you or DIY
...
#define NUM_EVENT ((NUM_SERVOS * (NUM_POS+4)) + ((NUM_NATIVE_IO + (NUM_MCP_GROUPS * NUM_MCP_IO_PER_GROUP))*2))
...
//#define REG_SERVO_EVENTS(s) CEID(servos[s].pos[0].eid), CEID(servos[s].pos[1].eid), CEID(servos[s].pos[2].eid), 
#define REG_SERVO_EVENTS(s) CEID(servos[s].pos[0].eid), CEID(servos[s].pos[1].eid), CEID(servos[s].pos[2].eid),\
                            PEID(servos[s].upFrogEid), PEID(servos[s].downFrogEid),PEID(servos[s].ClosedTargetEID),PEID(servos[s].ThrownTargetEID)
#define REG_IO(i) PCEID(io[i].onEid), PCEID(io[i].offEid)

#define _SERVOEID_1 REG_SERVO_EVENTS(0)
#define _SERVOEID_2 _SERVOEID_1, REG_SERVO_EVENTS(1)
#define _SERVOEID_3 _SERVOEID_2, REG_SERVO_EVENTS(2)
#define _SERVOEID_4 _SERVOEID_3, REG_SERVO_EVENTS(3)
#define _SERVOEID_5 _SERVOEID_4, REG_SERVO_EVENTS(4)
#define _SERVOEID_6 _SERVOEID_5, REG_SERVO_EVENTS(5)
#define _SERVOEID_7 _SERVOEID_6, REG_SERVO_EVENTS(6)
#define _SERVOEID_8 _SERVOEID_7, REG_SERVO_EVENTS(7)
#define _SERVOEID_9 _SERVOEID_8, REG_SERVO_EVENTS(8)
#define _SERVOEID_10 _SERVOEID_9, REG_SERVO_EVENTS(9)
