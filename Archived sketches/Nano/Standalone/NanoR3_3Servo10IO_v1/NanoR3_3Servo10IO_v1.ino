...
/*
This is my test version for demonstration Standalone use only by John Holmes 1 Servo 12 I/O
  ...
  - Pins 3,4,5,6,7,8,9,A0,A1,A2 are used for input or output
  - Pins A3,A4,A5 servos
...
#include "Config.h"  // Include user-configurable settings
...
#define MODEL "NanoR3_3Servo10IO"  // The model of the board
  ...
  CDIheader R"(
    ...
    <group replication=')" N(NUM_SERVOS) R"('>
        ...
        <group replication=')" N(NUM_POS) R"('>
            <repname>Closed</repname>
            <repname>Midpoint</repname>
            <repname>Thrown</repname>
...
extern "C" {
    ...
    const EIDTab eidtab[NUM_EVENT] PROGMEM = {
        REG_SERVO_OUTPUT(0), REG_SERVO_OUTPUT(1), REG_SERVO_OUTPUT(2),
        REG_IO(0), REG_IO(1), REG_IO(2), REG_IO(3), REG_IO(4), REG_IO(5), 
        REG_IO(6), REG_IO(7),REG_IO(8), REG_IO(9),   
    };
...
Servo servo[3];
uint8_t servodelay;
uint8_t servopin[NUM_SERVOS] = {A3,A4,A5};
uint8_t servoActual[NUM_SERVOS];
uint8_t servoTarget[NUM_SERVOS];

#ifdef ARDUINO_UNOR4_MINIMA
  uint8_t iopin[NUM_IO] = {2,3,4,6,6,8,9,A0,A1,A2; // Minima
#else
  uint8_t iopin[NUM_IO] = {3,4,5,6,7,8,9,A0,A1,A2}; // Nano
#endif
...
void userInitAll()
{ 
  NODECONFIG.put(EEADDR(nodeName), ESTRING("Nano R3"));
  NODECONFIG.put(EEADDR(nodeDesc), ESTRING("3 Servos 10 IO"));
