...
This is my test version for demonstration CAN BUS use only by John Holmes
  ...
  - Pins A0,A1,A2,A3,A4,A5 are used for servos

*/

//==============================================================
// AVR 6Servos 6IO ACAN with Toggle
//
// Coprright 2024 David P Harris
// derived from work by Alex Shepherd and David Harris
// 
//==============================================================
// - 6 Servo channels, each with 
...
#define MODEL "AVR6Servo6IO"   // The model of the board
...
#ifdef NOCAN
  uint8_t iopin[NUM_IO] = {4,5,6,7,8,9}; // use pin 13 LED for demo purposes with direct cnx
#else
  uint8_t iopin[NUM_IO] = {4,5,6,7,8,9};  // use free pins on MERG CAN board
#endif
...
{ 
  ...
  NODECONFIG.put(EEADDR(nodeDesc), ESTRING("6Servos6IO"));
  NODECONFIG.put(EEADDR(servodelay), 50);
  for(uint8_t i = 0; i < NUM_SERVOS; i++) {
    NODECONFIG.put(EEADDR(servos[i].desc), ESTRING(""));
    for(int p=0; p<NUM_POS; p++) {
      //NODECONFIG.writ16(EEADDR(servos[i].pos[p].angle), (uint8_t)((p*180)/(NUM_POS-1)));
