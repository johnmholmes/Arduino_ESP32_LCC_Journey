...
/*
This is my test version for demonstration CAN Bus use only by John Holmes 1 Servo 10 I/O
  ...
  - Pins 3,4,5,6,7,8,9,A1,A2,A3 are used for input or output
  - Pins A4 servos
...
#define MODEL "AVR1Servo10IO"  // The model of the board
...
Servo servo[1];
uint8_t servodelay;
uint8_t servopin[NUM_SERVOS] = {A4};
uint8_t servoActual[NUM_SERVOS];
uint8_t servoTarget[NUM_SERVOS];

#ifdef ARDUINO_UNOR4_MINIMA
  uint8_t iopin[NUM_IO] = {2,3,4,7,8,9,A0,A1,A2,A3}; // Minima
#else
  uint8_t iopin[NUM_IO] = {3,4,5,6,7,8,9,A1,A2,A3}; // Nano
#endif
...
{ 
  ...
  NODECONFIG.put(EEADDR(nodeDesc), ESTRING("1Servos10IO"));
