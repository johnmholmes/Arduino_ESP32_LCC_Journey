...
This is my test version for demonstration Standalone use only by John Holmes 1 Servo 12 I/O
  ...
  - Pins 3,4,5,6,7,8,9,A0,A1 are used for input or output
...
// Updated 2024.11.14
//==============================================================
// - 4 Servo channels, each with 
//     - three settable positions
//     - three set position events 
// - 9 input/output channels:
//     - type: 0=None, 
...
#define MODEL "NanoR3_4Servo9IO"  // The model of the board
...
Servo servo[4];
uint8_t servodelay;
uint8_t servopin[NUM_SERVOS] = {A2,A3,A4,A5};
uint8_t servoActual[NUM_SERVOS];
uint8_t servoTarget[NUM_SERVOS];

#ifdef ARDUINO_UNOR4_MINIMA
  uint8_t iopin[NUM_IO] = {2,3,4,6,6,8,9,A0,A1}; // Minima
#else
  uint8_t iopin[NUM_IO] = {3,4,5,6,7,8,9,A0,A1}; // Nano
#endif
...
{ 
  ...
  NODECONFIG.put(EEADDR(nodeDesc), ESTRING("4 Servos 9 IO"));
