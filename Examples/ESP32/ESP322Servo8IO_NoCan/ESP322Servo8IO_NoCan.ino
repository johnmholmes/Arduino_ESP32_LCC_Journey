...
ESP32 version only still being tested so may have bugs 5/1/25

This is my test version for demonstration NO CAN BUS use only by John Holmes

  - Pin 2 is used for interrupt
  - Pins 15,2,4,16,17,5,18,19 are used for input or output
  - Pins 12, 13 servos
...
#define MODEL "2Servo8IO_NoCan"   // The model of the board
...
#ifdef NOCAN
  uint8_t iopin[NUM_IO] = {15,2,4,16,17,5,18,19}; // use pin 13 LED for demo purposes with direct cnx
#else
  uint8_t iopin[NUM_IO] = {15,2,4,16,17,5,18,19};  // use free pins on MERG CAN board
#endif
