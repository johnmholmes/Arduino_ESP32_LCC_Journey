    ...
    typedef struct {
      ...
      // ===== Enter User definitions below =====
          ...
          struct {
            ...
            EventID ClosedTargetEID;
            EventID ThrownTargetEID;
          } servos[NUM_SERVOS];
          struct {
            char desc[8];
            uint8_t type;
...
void servoProcess() {
  ...
  static long lastmove = 0;      // ← Added this
  ...
  for (int i = 0; i < NUM_SERVOS; i++) {
    ...
    // Update lastmove whenever any servo moves
    lastmove = millis();

    // === Midpoint - Different event per direction ===
    uint8_t midpt = (NODECONFIG.read(EEADDR(servos[i].pos[0].angle)) +
                     NODECONFIG.read(EEADDR(servos[i].pos[NUM_POS-1].angle))) / 2;

    if (servoActual[i] == midpt) {
      dP(" ==> MIDPOINT");
      bool goingUp = (servoTarget[i] > servoActual[i]);
      OpenLcb.produce(i * NUM_SERVOS + NUM_POS + goingUp);     // +0 or +1 depending on direction
    }

    // === End positions ===
    uint8_t clpt = NODECONFIG.read(EEADDR(servos[i].pos[0].angle));
    uint8_t twpt = NODECONFIG.read(EEADDR(servos[i].pos[2].angle));


    if (servoActual[i] == twpt) {
      OpenLcb.produce(i * NUM_SERVOS + NUM_POS + 2);   // Thrown
    }
    if (servoActual[i] == clpt) {
      OpenLcb.produce(i * NUM_SERVOS + NUM_POS + 3);   // Closed
    }
  }

  // === Detach servos after 1 second of no movement ===
  ...
  // === Save current positions periodically ===
